...
struct Prediction <: Result end
struct Effect <: Result end

export Effect, Prediction

function errorplot!(ax, ::Effect, values; sigma=1)
    d = DataFrame(mean=mean(values), sd=std(values))
    sort!(d, :mean)
    errorbars!(ax, 1:nrow(d), d.mean, d.sd .* sigma)
    scatter!(ax, 1:nrow(d), d.mean; color=:black)
    ax
end
function errorplot(r::Effect, values; kwargs...)
    fig = Figure(); ax = Axis(fig[1,1])
    errorplot!(ax, r, values; kwargs...)
    fig
end
function errorplot(r::Result, m::AbstractDecisionModel, draws; kwargs...)
    fig = Figure(); ax = Axis(fig[1,1])
    errorplot!(ax, r, m, draws; kwargs...)
    fig
end

function errorplot!(
    ax, ::Prediction, m::AbstractDecisionModel, draws; 
    sigma=1, ylims=nothing, subset=nothing, sort=true
    )
	p = predict(m, eachrow(draws))
    d = DataFrame(mean=mean(p), sd=std(p))
    #e = empirical(m)
    
    d = isnothing(subset) ? d : d[subset,:]
    #e = isnothing(subset) ? e : e[subset]
    i = sort ? sortperm(d.mean) : 1:nrow(d)
    
    errorbars!(ax, 1:nrow(d), d.mean[i], d.sd[i] .* sigma; color=(:black, .6))
    scatter!(ax, 1:nrow(d), d.mean[i]; color=:black)
    #scatter!(ax, 1:nrow(d), e[i]; color=:black, marker=:xcross)
    isnothing(ylims) || ylims!(ax, ylims...)
    ax
end

function errorplot!(ax, ::Effect, ::BinomialGroupsModel, draws; sigma=1)
    s = sort!(DataFrame(mean=mean(draws.αs), sd=std(draws.αs)), :mean)
    errorbars!(ax, 1:nrow(s), s.mean, s.sd .* sigma)
    scatter!(ax, 1:nrow(s), s.mean; color=:black)
    ax
end
...
# 	Label(fig[1,1, TopLeft()], "A", fontsize=25, color=:black, padding=(0,15, 15, 0))
# 	Label(fig[2,1, TopLeft()], "B", fontsize=25, color=:black, padding=(0,15, 15, 0))
# 	Label(fig[3,1, TopLeft()], "C", fontsize=25, color=:black, padding=(0,15, 15, 0))
...
# 	 Label(fig[1,1, TopLeft()], "A", fontsize=25, color=:black, padding=(0,15, 15, 0))
# 	 Label(fig[1,2, TopLeft()], "B", fontsize=25, color=:black, padding=(0,15, 15, 0))
# 	 Label(fig[2,:, TopLeft()], "C", fontsize=25, color=:black, padding=(0,15, 15, 0)) 
# 	 Label(fig[3,:, TopLeft()], "D", fontsize=25, color=:black, padding=(0,15, 15, 0)) 
# 	 Label(fig[4,:, TopLeft()], "E", fontsize=25, color=:black, padding=(0,15, 15, 0)) 
