...
In terms of variation over time, there are slightly less dismissals in recent years.  


### Is there variation between boards?

```{julia}
df = map(summarize_outcome, group(label ∘ board, decisions)) |> DataFrame;
```
...
function plot_outcome_board(df)
    outcomes = [:dismissed, :partial, :annulled]
    dflong = DataFrames.stack(df, outcomes)
    dflong.value = dflong.value .* dflong.n
...
There is some slight variation in case outcomes across boards, with about a 7 percentage point difference in dismissal rates between, e.g., boards 3 and 4.
